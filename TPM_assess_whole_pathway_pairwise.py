...
__usage__ = """
					python TPM_assess_whole_pathway_pairwise.py
					--exp1 <BV_TPM_FILE>
				...
				for idx, val in enumerate( parts[1:] ):
					if not "nan" in val:
						...
						try:
							...
							try:
								exp[ name ].update( { samples[ idx ]: val } )
							except KeyError:
								exp.update( { name: { samples[ idx ]: val } } )
			except KeyError:
				pass
...
def generate_data_matrix_file( data, samples, output_file ):
	"""! @brief generate a data matrix for heatmap construction """
	
	normed_data = {}	#B lineage
	with open( output_file, "w" ) as out:
		genes = [ "CHS", "CHI", "F3H", "F3-H", "F3-5-H", "DFR", "ANS", "LAR", "ANR" ]
		for gene in genes:
...
def generate_pathway_overview_figure( bv_normed_data, at_normed_data, figurefile, species1, species2 ):
	...
	ax.set_ylabel( "gene expression [TPM]", fontsize=16 )
	...
	plt.subplots_adjust( left=0.09, right=0.99, top=0.99, bottom=0.075 )
...
def main( arguments ):
	...
	bv_normed_data = generate_data_matrix_file( bv_exp_data, bv_samples, bv_output_file )
	
	at_output_file = output_folder + species2 + "_data_matrix_for_heatmap.txt"
	at_normed_data = generate_data_matrix_file( at_exp_data, at_samples, at_output_file ) 
	
	# --- generate boxplot overview figure --- #
	overview_boxplot_figure = output_folder + species1.replace( " ", "_" ) + "_vs_" + species2.replace( " ", "_" ) + "_boxplot.pdf"
	generate_pathway_overview_figure( bv_normed_data, at_normed_data, overview_boxplot_figure, species1, species2 )
