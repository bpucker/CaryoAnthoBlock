...
__usage__ = """
					...
					--roi <SEMICOLON_SEP_GENES_COMMA_SEP_RESIDUES_OF_INTEREST>[False]
					"""
...
def get_info_per_species( inputfile ):
	...
	with open( inputfile, "r" ) as f:
		residues = f.readline().strip().split('\t')[1:]	#remove header
		line = f.readline()
		while line:
			parts = line.strip().split('\t')
			tmp = {}
			for idx, part in enumerate( parts[1:] ):
				tmp.update( { residues[ idx ]: part } )
			infos.update( { parts[0]: tmp } )
			line = f.readline()
	return infos, residues
...
def main( arguments ):
	...
	if '--roi' in arguments:
		all_residues_of_interest = arguments[ arguments.index( '--roi' )+1 ]
		if ";" in all_residues_of_interest:
			gene_parts = all_residues_of_interest.split(';')
		else:
			gene_parts = [ all_residues_of_interest ]
